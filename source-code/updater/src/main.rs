...
fn get_root_device() -> Result<String> {
    ...
    let device = output.stdout.trim().replace(r#"\[.*\]"#, "");
    Ok(device)
}
...
fn snapshot_recursive(source: &str, dest: &str, writable: bool) -> Result<()> {
    ...
    for line in list_output.stdout.lines() {
        if let Some(captures) = regex::Regex::new(r"ID \d+ gen \d+ path (.*)").unwrap().captures(line) {
...
fn init_command() -> Result<()> {
    let mut new_deployment = None;
    let mut temp_chroot = None;
    let mut temp_mounted = false;
    let mut chroot_mounted = false;
    ...
    new_deployment = Some(new_deployment_path.clone());
    ...
    temp_chroot = Some(temp_dir.clone());
    ...
    let chroot_cmd = format!("chroot {} /bin/sh -c 'apt update && apt install --reinstall -y plymouth && apt-mark manual plymouth && dpkg -l > /tmp/packages.list && update-initramfs -u -k all && chmod -x /etc/grub.d/10_linux /etc/grub.d/20_linux_xen /etc/grub.d/30_os-prober'", temp_dir);
...
fn update_command() -> Result<()> {
    ...
    let chroot_cmd = format!("chroot {} /bin/sh -c 'apt update && apt-mark manual plymouth && apt upgrade -y -o Dpkg::Options::=--force-confold && apt autoremove -y && dpkg -l > /tmp/packages.list && update-initramfs -u -k all && chmod -x /etc/grub.d/10_linux /etc/grub.d/20_linux_xen /etc/grub.d/30_os-prober'", temp_dir);
    let output = run_command("/bin/sh", &["-c", &chroot_cmd])?;
    if !output.success {
...
fn bind_mounts_for_chroot(chroot: &str, mount: bool) -> Result<()> {
    ...
    for bind in binds {
        ...
        let args = if mount { vec!["--bind", bind, &target] } else { vec![&target] };
...
fn get_kernel_version(chroot: &str) -> Result<String> {
    // Zakładam implementację: przeczytaj /boot/vmlinuz lub coś
    // Na przykład:
...
fn sanity_check(_deployment: &str, _kernel: &str, _chroot: &str) -> Result<()> {
    // Implementuj sanity checks jeśli potrzeba
    Ok(())
}

fn compute_system_version(deployment: &str) -> Result<String> {
    let mut hasher = Sha256::new();
    // Przykładowo hash packages.list
    let packages_path = format!("{}/tmp/packages.list", deployment);
...
fn update_bootloader_entries(_deployment: &str) -> Result<()> {
    // Implementuj jeśli potrzeba
    Ok(())
}

fn set_status_broken(deployment: &str) {
    // Edytuj meta.json na status "broken"
}
